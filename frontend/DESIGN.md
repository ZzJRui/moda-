...
> 冻结区 = 经过大量调校、已是生产级观感的区域。只允许**结构性重构**
> （抽组件、换样式载体、值 1:1 保留），**任何改变几何/观感的调整都禁止**，
> 包括间距、圆角、动效、Swiper 参数、高度算法。视觉打磨绕开这些区域。

- **搭配页 3D 单品老虎机（核心）**：`flows/ai-styling/ClothingCarousel.tsx` +
  `ClothingCarousel.module.css` 整体，**以及** `ai-styling.tsx` 中驱动其几何的部分——
  `captureWrap` 内边距、`scrollArea` 高度与 `cardHeight` 动态算法、三条 slot 的间距。
  这套 Swiper 参数（`slideSize=100` / `trackOffset=0` / 每片 40px 内边距）与高度算法
  是反复调出来的 3D 透视效果，视为冻结生产代码，一个像素都不许动。
  搭配页的其余外壳（页头、保存按钮、"帮你搭" FAB、AI 对话卡片、动画遮罩）不在冻结范围，可打磨。
- **收藏页"搭配详情"视图**（`flows/favorites` 中 `selectedDetail` 分支，页头标题为"搭配详情"）：
  只允许结构性重构，像素观感必须保持不变（已用 `frozen*` 类名标记）。
